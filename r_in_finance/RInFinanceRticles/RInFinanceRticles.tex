...
is in the ability to scale analyses using the \emph{split-apply-combine}
framework. Because of the separation in development, the two systems are
...
``tidy'' ecosystem. The integration unlocks the
\emph{split-apply-combine} framework to scale analyses.

Two usage cases are discussed to illustrate the benefits of
...
reward. The second example evaluates methods to scale the performance
analysis of multiple portfolios using combinations of weighted asset
blends. These examples only scratch the surface of what is possible with
\texttt{tidyquant}.
}
...
\texttt{tidyquant} unlocks the \emph{split-apply-combine} framework,
increasing the scale at which financial analyses can now be performed.
As the scale of analyses increase, the vast array of possibilities
becomes limited only by the data storage and processing capabilities of
the computers on which the computations are performed. With advances in
cloud technology, parallel processing, and optimization algorithms, the
tools are available to scale analyses to terabytes worth of financial
data in real time. The work is not complete, but as innovation continues
financial analysis will benefit from both speed and scale.
