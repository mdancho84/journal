...
abstract: >
  Financial analysis and data science in the R programming language have followed two separate yet innovative paths resulting in two different but important systems, "xts" and "tidyverse". Because of the separation in development, the two systems are difficult to use together, which limits the full potential of financial analysis within R. The `tidyquant` package is an attempt to solve this problem by integrating the best financial analysis packages in the "xts" world with the "tidy" ecosystem, unlocking the benefits of the _split-apply-combine_ framework and enabling the ability to scale financial analyses. 

    Two usage cases are discussed to illustrate the potential. The first example uses the scaling capabilities to provide an answer for how the "market" values risk versus reward. The second example evaluates the performance of multiple portfolios using weighted blends. These examples just scratch the surface of the full potential as technology develops, and future possibilities are briefly addressed. 
preamble: >
...
In the next section, the _split-apply-combine_ framework is discussed conceptually using non-financial data and then a demonstration of the `tidyquant` package is presented to illustrate some of the key benefits of the integration of "xts" based financial packages into the "tidy" ecosystem.
