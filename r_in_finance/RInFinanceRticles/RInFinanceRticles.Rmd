...
The two systems, "xts" and the "tidyverse", have advantages that are needed within the realm of financial analysis. The "xts" system has the infrastructure to perform financial analytics. The "tidyverse" has the functions and data representation to seamlessly scale analysis using the data science workflow. Unfortunately, the two systems do not work well together. The "xts" system has underlying dependencies on the "matrix" data structure whereas the "tidy" ecosystem is built on top of the "tibble" ("tidy" data frame) data structure. The "matrix" structure is _homogenous_ meaning each column in the frame must hold the same data type. In R, coercion rules takeover when data types are combined within a "matrix". For example, combining a character and a numeric class will result in a matrix of character class. As a result, the "xts" system can not be formatted in a "tidy" structure with each variable as a column, each observation as a row, and each value in a cell. The solution is to merge data sets together keeping the column names as the observation. This works well in the "xts" system, but presents problems in the "tidyverse" system since the data is not represented following the "tidy" principles. Conversely, the "tibble" structure is _heterogenous_ meaning each column can contain a different data type. This works extremely well for grouping observations together. For example, a "tibble" can contain symbols in one column and daily prices in another column. This enables applying functions to groups (e.g. symbol) for scaling analyses. The end result is two excellent systems that do not communicate easily. Without communication between each system the full potential of financial analysis within R is limited.
...
Example 1 can be expanded on the notion that volatility is a factor that the market tends to penalize. In this example, the goal is to visualize returns of the top ranked prospects from Example 1 to determine if the group is consistently growing. This example will introduce a new package called `purrr`, which is used to map functions to "nested" data frames. 

Start with a question: _Of the best stocks from Example 1, which offer the most consistent growth?_ 

The easiest way to answer this question is to visualize the mean daily log returns (MDLR) by year. Collecting the means by year can be accomplished a number of ways. The `purrr` package offers a new approach by applying functions to data frames similar to the `apply` function in base R. The nice thing about `map` is that if a function can be applied to one group of the data, it can be "mapped" to many groups. This example follows the followings steps:
...
high_performers <- sp500_high_performers %>%
    ungroup() %>%
    select(symbol, ArithmeticMean) %>%
    top_n(n = n) %>%
    .$symbol
high_performers
```

Next, filter the SP500 returns data frame to only the stocks of interest and "nest" the data frame. Nesting works on groups, so all of the data that is grouped on gets rolled into a list column.  


```{r}
high_performer_returns_nested <- sp500_returns %>%
    filter(symbol %in% high_performers) %>%
    nest()
high_performer_returns_nested
```

Once the data frame is nested, create a custom function to apply to each of the nested tibbles. The function `calculate_mdlr_by_year` transforms the daily log returns to mean daily log returns by year. It takes a set of daily log returns and returns the mean daily log returns by year.
...
high_performer_returns_mapped %>%
    ...
    labs(title = "Comparing the Return Consistency of High Performers",
         y = "Mean Daily Log Returns (MDLR)", color = "") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          axis.title.x = element_blank())
```

The mean daily log returns when averaged by year yields a different picture that what the results of Example 1 portrayed. Some of the stocks are very consistent while others have downward trends or have negative returns in multiple years indicating investment losses. 

Further, with the data science modeling workflow discussed in _R for Data Science_, a financial analyst or engineer can create screening metrics by running regressions for slope and intercept (refer to `broom` package), number of times an investment falls below zero, standard deviation in MDLR by year, and more. Screening investments within an entire index or exchange becomes relatively easy even with complex metrics.   


## Example 3: Evaluating Performance of Multiple Portfolio Blends

Portfolio aggregation is a useful technique to reduce risk while maintaining acceptable returns. In this example, the goal is to evaluate a few blended portfolios of "FANG" stocks ("FB", "AMZN", "NFLX", and "GOOG"). However, more important is the conceptual idea that the number of weighted portfolio blends and the number of underlying assets can easily be increased to scale the analysis. As the example progresses, consider how this could be applied to an entire index of assets and hundreds of weighted blends.  
...
Use `tq_repeat_df` to repeat `n = 4` times. This function extends the data frame row-wise, adding an index column named "portfolio" and grouping by "portfolio". Four portfolio groups are generated. 
